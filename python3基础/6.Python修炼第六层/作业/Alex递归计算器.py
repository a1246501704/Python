...
def fetch_data_from_bracket(data_list, first_right_bracket_pos):
    ...
    print
    '\033[31;1mleft bracket pos:%s right_bracket_pos: %s\033[0m' % (left_bracket_pos, first_right_bracket_pos)
    data_after_strip = data_list[left_bracket_pos:right_bracket_pos]

    if data_after_strip.count("(") > 1:
        print
        'fetch_data_from_bracket:%s \033[31;1m%s\033[0m left pos:%s' % (
        data_after_strip, data_after_strip[1:], left_bracket_pos)
        ...
        print
        'last:', len(data_after_strip), data_after_strip
...
def parse_bracket(formula):  # 解析空格中的公式
    ...
    pattern = r"\(.+\)"
    m = re.search(pattern,
                  formula)  # 匹配出所有的括号 ‘3 / 1      - 2 * ( (60-30 * (4-2)) - 4*3/ (6-3*2) )’ 匹配完之后是'( (60-30 * (4-2)) - 4*3/ (6-3*2) )'
    if m:
        ...
        print
        '\033[32;1mResult:\033[0m', calc_res
        print
        calc_res[1], calc_res[2]
        print
        data_with_brackets[calc_res[1]:calc_res[2]]
        ...
        print
        '\033[42;1mCaculation result:\033[0m', formula
...
def caculate_2(data_list, operator_list):
    ...
    print
    'caculater_2:', data_list, operator_list
...
def parse_operator(formula):
    ...
    print
    '去掉加减后的公式列表,先算乘除:', data_after_removed_low_priorities
    ...
    print
    '---------->handle nagative num:', data_after_removed_low_priorities
    # 计算乘除运算
    nagative_mark = False
    ...
    print
    '去掉* 和 /后开始运算加减:', data_after_removed_low_priorities, low_priorities
    # 计算加减运算
...
def main():
    while True:
        ...
        print
        '\033[43;1mpython计算器运算结果:\033[0m', eval(user_input)
