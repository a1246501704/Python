...
1 交叉连接：不适用任何匹配条件。生成笛卡尔积(用第一张表的每一条记录和第二张表的所有记录做了对应关系，其中肯定有对的一条记录)
...
mysql> select * from employee,department where employee.dep_id = department.id; # 找到一个两张表唯一正确的判断依据
+----+-----------+--------+------+--------+------+--------------+
| id | name      | sex    | age  | dep_id | id   | name         |
+----+-----------+--------+------+--------+------+--------------+
|  1 | egon      | male   |   18 |    200 |  200 | 技术         |
...
5 全外连接：显示左右两个表全部记录
全外连接：在内连接的基础上增加 左边有右边没有的 和 右边有左边没有的结果
...
\四 子查询
#1：子查询就是将一个查询语句的结果用括号扩起来当作另一个查询语句的条件取用。
...
# 查询 技术部和人力资源不的人
select * from employee where dep_id in (select id from department where name = "技术" or name = "人力资源");
+----+-----------+--------+------+--------+
| id | name      | sex    | age  | dep_id |
+----+-----------+--------+------+--------+
|  1 | egon      | male   |   18 |    200 |
...
# 注意：括号的其他用途
# 这样就生成了一张虚拟表，通过这样方法可以实现一张表和自己实现多表查询，也可以实现更多的表之间查询
select * from (select id from department where name = "技术" or name = "人力资源") as t1
