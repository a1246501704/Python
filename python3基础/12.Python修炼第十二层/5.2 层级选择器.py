...
2、子元素选择器只会查找儿子（同级有多个都算），不会查找其他嵌套的标签（不找孙子、重孙子）。
...
\3、相邻兄弟选择器，CSS2推出（加号连接）
#1、作用：选定紧跟其后的那一个标签，且二者有相同父元素。
...
#3、注意点：
1、相邻选择器必须通过+号链接，选择器1标签相邻的选择器2标签才会被选中。
2、相邻选择器只能选中紧跟其后的那一个标签，不能选中被隔开的标签。
...
\4、通用兄弟选择器，CSS3推出（波浪号）
#1、作用：指定本级的所有选择器中的所有标签
...
#3、注意点：
1、通用兄弟选择器必须用~来链接
2、通用兄弟选择器选中的是指 选择器1 后面同级的所有 选择器2的标签会被选中，匹配到 选择器1 同级所有的标签为 选择器2的标签。
