...
1、浮动流是一种 半脱离标准流的排版方式 那什么是脱离文档流？什么又是半脱离文档流？
\1.1 什么是脱离文档流？
1、浮动元素脱离文档流意味着
    #1、不再区分 行内、块级、行内块级，无论是什么级的元素都可以水平排版
    #2、无论是什么级的元素都可以设置宽和高
    ...
    <head>
        ...
        <style>
            * {
            ...
            /*
            不再区分 行内、块级、行内块级，无论是什么级的元素都可以水平排版：span和p都显示到一行
...
示范2
案例的核心原理：既然div2没有设置浮动，那么肯定独占一行，所以div3只能在div2下面这一行（浮动除非是被迫顶上去，
             否则不可能改变上下位置，只能在当前行的左右漂浮）的左右浮动！
<!DOCTYPE html>
<html lang="en">
    <head>
...
2、浮动元素脱标文档流意味着
    ...
    #2、如果前面一个元素浮动走了，而后面一个元素没有浮动，那么垂直方向的元素会自动填充它的位置，浮动元素再重新归位后就会覆盖该元素。
...
# 让多个块级元素显示到一行，有两种实现方式，一种是修改元素的显示方式为 inline-block，另外一种就是用浮动的方式
<!DOCTYPE html>
<html>
    <head>
...
## 浮动元素浮动之后的位置取决于它在浮动之前的标准流中的位置（如果有没浮动的元素在上面，半脱离文档流的浮动元素要在其下面）
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8">
...
# 1.3 浮动元素贴靠问题
当父元素的宽度足够显示所有元素时，浮动的元素就会并列显示. 注：只适用于宽度，高度不够就直接超出到父标签外面显示了。
当父元素的宽度不够显示所有元素时，放不下的浮动元素就贴前一个元素排列显示，如果还不够，就会再贴前一个元素排列显示。
直到贴到父元素左边，此时无论是否宽度足够都会在这一行显示了（自己新起了一行）。
...
# 1.4 浮动元素字围现象
浮动的文字、图片、超链接等元素不会给浮动的元素让位置，也不会围绕在浮动元素的周围。
...
# 1.6 浮动元素高度问题(又称父级塌陷)
    ...
    #2、在浮动流中,浮动的元素是不可以撑起父元素的高度的，当子元素都浮动起来后，父亲的内容高度即 height变为0(空了)，父元素就好像塌陷了一样，因而又称为父级塌陷.
...
清除浮动方式一：为浮动的那些子元素的父级标签设置一个高度（将父级撑起来）
...
清除浮动方式二： 使用 clear 方法清除浮动带来的影响（并不是清除其他浮动元素的浮动效果）
    clear: left
...
#2、把握住两点：
    ...
    3、clear 这个属性必须设置在块级、并且不浮动的元素中设置。
...
清除浮动的方式四：伪元素选择器（广泛使用）
#1、复习伪元素选择器(CSS3中新增的伪元素选择器)
    伪元素选择器的作用就：是给指定标签的内容前面添加一个子元素 或者 给指定标签的内容后面添加一个子元素
...
#2、格式：给指定标签的前面添加子元素
  ...
  # 格式：给指定标签的后面添加子元素
    ...
    本质原理与内墙法一样，但我们用的css的伪元素选择器实现的，就应该用css来控制样式，符合前端开发思想
            #I、详细用法
            .header:after {                   <----在类名为“.header”的元素内部最后面加入以下内容；
        ...
        <style>
            * {
            ...
            .box1 {
                ...
                /*border: 1px solid #000;*/  # 设置了overflow 父盒子不加边框也不会被往下顶
                overflow: hidden;
            }
            .box2 {
                width: 100px;
...
## relative（翻译：日无哎  类提舞）（相对定位-相对自己原来在的位置）
相对定位是相对于该元素在文档流中的原始位置，即以自己原始位置为参照物。有趣的是，即使设定了元素的相对定位以及偏移值，元素还占有着原来的位置，即占据文档流空间。
对象遵循正常文档流，但将依据top（上），right（右），bottom（下），left（左）等属性在正常文档流中偏移位置。而其层叠通过z-index属性定义。
注意：position：relative 的一个主要用法：方便绝对定位元素找到参照物。

## absolute（绝对定位-相对离它最近的已经定位过的父标签）
定义：设置为绝对定位的元素框会从文档流完全删除，并相对于最近的已经定位的父级元素再定位，如果元素的父级元素没有定位，就会往上继续找已经定位过的父级标签，如果都没有那么它的位置就会相对于最初的包含块（即body元素）。元素原先在正常文档流中所占的空间会关闭，就好像该元素原来不存在一样。元素定位后生成一个块级框，而不论原来它在正常流中生成何种类型的框。
...
另外，对象脱离正常文档流，使用top，right，bottom，left等属性进行绝对定位。而其层叠通过z-index（指定元素层叠顺序）属性定义。
...
　　top/bottom 同时只能用一个，要么上面让出位置 要么下面让出位置。
　　left/right 同时只能用一个，要么左边让出位置 要么右边让出位置。
...
2、绝对定位- （相对于body或者某个定位流中的父级元素来定位）
# 例如一个需要做绝对定位的标签A，如果它的父级标签没有设置定位，那么这个A标签的定位就相对于body标签来移动。
# 如果需要做绝对定位的标签A的父级没有绝对定位，还可以依次找父级的父级，父级都没有再以body为准。
# 如果A标签需要做绝对定位，并且以它的父级为相对位置，那么这个父级的标签要在定位流中。父标签需要加 position: relative; 属性就是在定位流中了。
...
<html lang="en">
    <head>
        ...
        <style>
            ...
            .box2 {
                position: absolute; # 绝对移动，看着像是浮动起来之后又做的移动。因为没有父级做了定位所以是相对于body做的定位，所以看着像是浮动了。
                /*left: 0;*/
                /*top: 10px;*/
                background-color: green;
...
2.1 绝对定位的参考点
#1、默认情况下所有的绝对定位的元素，无论有无祖先元素，都会以body大小作为参考点移动。
#2、如果一个绝对定位的元素有祖先元素，并且祖先元素也在定位流，那么这个绝对定位的元素就会以定位流的那个祖先元素作为参考点移动。
    2.1.1 只要是这个绝对定位元素的祖先元素都可以
    2.2.2 祖先必须是定位流，此处的定位流指的是绝对定位、相对定位、固定定位(定位流中只有静态定位不行)
    2.3.3 如果一个绝对定位的元素有祖先元素，而且祖先元素中有多个元素都是定位流，那么这个绝对定位的元素会以离它最近的那个定位流的祖先元素为参考点
...
#2.如果想让过一个绝对定位的盒子自身居中, 可以使用left: 50%; margin-left: -元素宽度的一半px;
...
背景的关联方式 background-attachment: fixed;可以让图片不随着滚动条的滚动而滚动
...
\z-index
#1、z-index属性：用于指定定位流的元素的覆盖关系
    1.1、z-index值表示谁压着谁。数值大的压盖住数值小的。
    1.2、只有定位了的元素，才能有z-index值。也就是说，不管是相对定位、绝对定位、固定定位，都可以使用z-index值。而浮动的元素不能用。
    1.3、z-index值没有单位，就是一个正整数。默认的z-index值是0。
    1.4、如果大家都没有z-index值（默认所有元素z-index值为0），或者z-index值一样，那么谁写在HTML后面，谁在上面能压住别人。
         定位了的元素，永远能够压住没有定位的元素。

#2、注意点：从父现象（父亲怂了，儿子再牛逼也没用）
    父元素没有z-index值, 那么子元素谁的z-index大，其他的就被覆盖。
    父元素z-index值不一样, 那么父元素谁的z-index大，其他的就被覆盖。
