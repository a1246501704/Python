...
def create_classes():
    try:
        ...
        if name in classes_name_list:
            raise Exception('\033[43;1m班级[%s] 已经存在，不可重复创建\033[0m' %(name))
        obj=Classes(name)
        obj.save()
        status=True
        error=''
        data='\033[33;1m 班级[%s] 时间[%s]创建成功\003[0m' %(obj.name,obj.create_time)
    except Exception as e:
        status=False
        error=str(e)
        data=''
...
def show_classes():
    for obj in Classes.get_all_obj_list():
        print('\033[33;1m 班级[%s] 创建时间[%s]\033[0m'.center(60, '-') \
              %(obj.name,obj.create_time))
...
def create_student():
    try:
        ...
        student_name_list=[(obj.name,obj.age,obj.qq) for obj in Student.get_all_obj_list()]
        if name in student_name_list:
            raise Exception('\033[43;1m姓名[%s] 已经存在，不可重复创建\033[0m' %(name))
        obj=Student(name,age,qq)
        obj.save()
        status=True
        error=''
        data='\033[33;1m 姓名[%s] 年龄[%s] QQ[%s] 时间[%s]创建成功\003[0m' \
             %(obj.name,obj.age,obj.qq,obj.create_time)
    except Exception as e:
        status=False
        error=str(e)
        data=''
...
def show_student():
    for obj in Student.get_all_obj_list():
        print('\033[33;1m 姓名[%s] 年龄[%s] QQ[%s] 创建时间[%s]\033[0m'.center(60, '-') \
              %(obj.name,obj.age,obj.qq,obj.create_time))
