...
def producer(name,food,q):
    ...
    # q.put(None) # 如果有多个消费者会出错，队列中间会有None。配合44行使用。
# 消费者
def consumer(name,q):
    while True:
        res=q.get()
...
if __name__ == '__main__':
    ...
    p1.join() # 把生产者join住，保证生产完在加入None。
    p2.join()
    q.put(None) # 多个消费者时，生产者生产完了放入几个None，给三个消费者每人一个None。方法很low，有几个消费者就要传一个None。
    q.put(None)
    q.put(None)
...
if __name__ == '__main__':
    ...
    c1.daemon=True # 把消费者变为守护进程，当q.join执行结束时已经确定生产者都生产完了，消费者已经消费完了。所以消费者也可以退出了。
    c2.daemon=True
    c3.daemon=True
    ...
    p1.join()
    p2.join()
    q.join() # 上面的start执行完会直接到此行代码，还没消费就结束了。所以要把p1和p2都join住。保证生产完毕再q.join住才有意义。
    print('主')
